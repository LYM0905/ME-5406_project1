...
if __name__ == '__main__':
    ...
    if args.job == 0:
        ...
        Monte_carlo = Monte_carlo(env, epsilon=0.9, gamma=0.99)

        SARSA = SARSA(env, learning_rate=0.8, gamma=0.99, epsilon=0.85)

        Q_learning = Q_learning(env, learning_rate=0.9, gamma=0.7, epsilon=0.75)
    ...
    # Job 1, 10X10 frozen lake environment training, correctness test, and comparison test
        ...
        # Create three agents corresponding to three algorithms
        Monte_carlo = Monte_carlo(env, epsilon=0.31, gamma=0.9999)

        SARSA = SARSA(env, learning_rate=0.1, gamma=0.99, epsilon=0.95)

        Q_learning = Q_learning(env, learning_rate=0.8, gamma=0.7, epsilon=0.05)

        label_1 = ['Monte_carlo', 'SARSA', 'Q_learning']

        Q_1, steps_1, all_cost_1, accuracy_1, all_cost_bar_1, Rewards_list_1 = Monte_carlo.fv_mc_prediction(num_epoch=30000)

        Q_2, steps_2, all_cost_2, accuracy_2, all_cost_bar_2, Rewards_list_2 = SARSA.train(num_epoch=20000)

        Q_3, steps_3, all_cost_3, accuracy_3, all_cost_bar_3, Rewards_list_3 = Q_learning.train(num_epoch=10000)
    ...
    # Job 2, comparison test for different learning rate value settings(defaut for 4*4)
    elif args.job == 2:
    ...
    # Job 3, comparison test for different gamma value settings(defaut for 4*4)
    elif args.job == 3:
        env = Environment(grid_size=args.grid_size)
    ...
    # Job 4, comparison test for different epsilon value settings(defaut for 4*4)
    elif args.job == 4:
        env = Environment(grid_size=args.grid_size)
