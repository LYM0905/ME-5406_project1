...
class Q_learning(object):
    ...
    def check_state_validation(self, state):
        if state not in self.q_table.index:
            # 创建一个字典，其中每个动作的Q值初始化为0
            new_row = {action: 0 for action in self.actions}
            # 将字典转换为DataFrame的一行
            new_state_df = pd.DataFrame(new_row, index=[state])
            # 使用pd.concat来合并原始的q_table和新的一行
            self.q_table = pd.concat([self.q_table, new_state_df])
    ...
    # Learning and updating Q-table using Q-learning formula
